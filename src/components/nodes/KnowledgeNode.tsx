...
import { Database, Copy, Trash2, Globe, FileText, Search, Plus } from 'lucide-react';
import { useWorkflowStore } from '../../store/useWorkflowStore';
import NodeExecutionBadge from './NodeExecutionBadge';

export default function KnowledgeNode({ selected, id, data }: NodeProps) {
  const [hovered, setHovered] = useState(false);
  const { deleteNode, duplicateNode, setInsertionContext } = useWorkflowStore();

  const actionType = data?.actionType as string || 'search_vector_db';
  
  let Icon = Database;
  let label = 'Knowledge Base';
  let color = '#10b981';

  if (actionType === 'scrape_url') { Icon = Globe; label = 'Scrape URL'; color = '#3b82f6'; }
  else if (actionType === 'parse_pdf') { Icon = FileText; label = 'Parse PDF'; color = '#ef4444'; }
  else if (actionType === 'upsert_to_vector_db') { Icon = Database; label = 'Save Vector'; color = '#10b981'; }
  else if (actionType === 'search_vector_db') { Icon = Search; label = 'Search Vector'; color = '#8b5cf6'; }
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', paddingRight: '4px', overflow: 'hidden' }}>
          ...
          <span style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.2 }}>KNOWLEDGE</span>
        </div>
      </div>
