...
async function runStructuredPrompt(apiKey: string, systemInstruction: string, userPrompt: string): Promise<string> {
...
// ─── AI Utilities Piece ──────────────────────────────────────────────────────
export const aiUtilsPiece = createPiece({
  name: 'ai_utils',
  displayName: 'AI Utilities',
  logoUrl: '',
  actions: {
    sentiment_analysis: createAction({
      ...
      run: async (context) => {
        ...
        const result = await runStructuredPrompt(apiKey,
          `You are a sentiment analysis engine. Analyze the following text and respond with ONLY a JSON object containing:
...
Do not include any other text, only valid JSON.`,
    ...
    classify_text: createAction({
      ...
      run: async (context) => {
        ...
        const result = await runStructuredPrompt(apiKey,
          `You are a text classification engine. Classify the following text into one or more of these categories: ${categories}.
...
Do not include any other text, only valid JSON.`,
    ...
    extract_entities: createAction({
      ...
      run: async (context) => {
        ...
        const result = await runStructuredPrompt(apiKey,
          `You are a named entity extraction engine. Extract all named entities from the following text.
Respond with ONLY a JSON object containing:
...
Do not include any other text, only valid JSON.`,
    ...
    summarize_text: createAction({
      ...
      run: async (context) => {
        ...
        const result = await runStructuredPrompt(apiKey,
          `You are a text summarization engine. Create a ${lengthGuide} summary of the following text.
Respond with ONLY a JSON object containing:
...
Do not include any other text, only valid JSON.`,
    ...
    translate_text: createAction({
      ...
      run: async (context) => {
        ...
        const result = await runStructuredPrompt(apiKey,
          `You are a translation engine. Translate the following text to ${targetLanguage}. 
Respond with ONLY a JSON object containing:
...
Do not include any other text, only valid JSON.`,
        ...
        try {
          ...
          return { ...parsed, generatedText: parsed.translation };
        } catch {
